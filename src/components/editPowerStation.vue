...
export default {
  ...
  methods: {
    ...
    locatePowerStation() {
      ...
      // 建立新增电站矢量图层
      const powerStationVectorLayer = new VectorLayer({
        source: new VectorSource(),
      });
      powerStationVectorLayer.getSource().addFeatures(allPowerFeature);
      this.map.addLayer(powerStationVectorLayer);
      powerStationVectorLayer.setStyle(this.powerStationStyle);
      this.locate(allPowerFeature);
      const selectPoint = this.createSelectInteraction([
        powerStationVectorLayer,
      ]);
      this.map.addInteraction(selectPoint);
      this.addSelectEvent(selectPoint, this.popup);
    },
    //新增电站图层样式
    powerStationStyle() {
      return new Style({
    ...
    addSelectEvent(interaction, popup) {
      interaction.on("select", (event) => {
        ...
        if (feature && !this.isMultiClusterFeature(feature)) {
          ...
          console.log("powerCotent", powerCotent);
          this.hidePopup();
